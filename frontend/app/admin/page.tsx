...
import { ensureAuthDev, getTopics, getLessons, createTopic, deleteTopic, createLesson, deleteLesson } from "../../lib/api";
...
const ICONS = [
  { id: "objections.svg", label: "Обработка возражений" },
  { id: "closing.svg", label: "Закрытие сделки" },
  { id: "postmeet.svg", label: "После встречи" },
  { id: "needs.svg", label: "Выявление потребностей" },
...
// Backend integration replaces localStorage persistence
...
export default function Page() {
  ...
  useEffect(() => {
    (async () => {
      // Ensure local dev auth, then load data
      await ensureAuthDev();
  ...
  const addLesson = async () => {
    ...
    const created = await createLesson({
      ...
      icon: lessonIcon,
      topicId: lessonTopicId ? Number(lessonTopicId) : null,
    });
